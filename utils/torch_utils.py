...
def fuse_conv_and_bn(conv, bn):
    ...
    # print("conv:",w_conv.shape,"bn:",w_bn.shape)
...
def model_info(model, verbose=False, img_size=640):
    ...
    return flops * img_size[0] / stride * img_size[1] / stride, len(list(model.modules())),n_p, n_g,fs

def info(model, verbose=False, img_size=640):
    # Model information. img_size may be int or list, i.e. img_size=640 or img_size=[640, 320]
    n_p = sum(x.numel() for x in model.parameters())  # number parameters
    n_g = sum(x.numel() for x in model.parameters() if x.requires_grad)  # number gradients
    stride = 32
    ...
    # LOGGER.info(f"Model Summary: {len(list(model.modules()))} layers, {n_p} parameters, {n_g} gradients{fs}")
    return "Model Summary: "+ str(len(list(model.modules()))) +" layers, " + str(n_p) +" parameters, "+str(n_g) + " gradients "+ str(fs)
