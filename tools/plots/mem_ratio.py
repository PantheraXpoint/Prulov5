...
for ff in directory_path.glob('*'):
    ...
    if 'hardware' in file:
        ...
        df['Combined Memory'] = df['GPU Memory Usage (MB)'] + df['RAM Usage (MB)']

        # Calculate the mean RAM usage for each PID
        mean_ram_usage = df.groupby('PID')['Combined Memory'].transform('mean')

        # Calculate the RAM usage difference with the mean
        df['Combined Memory Usage Diff'] = df['Combined Memory'] - mean_ram_usage

        # Filter out rows where RAM usage difference is greater than 10 MB
        filtered_df = df[df['Combined Memory Usage Diff'].abs() <= 50]

        # Drop the 'RAM Usage Diff' column as it's no longer needed
        filtered_df = filtered_df.drop(columns=['Combined Memory Usage Diff'])
...
for idx,(pruning_ratio, group) in enumerate(grouped):
    ...
    line, = plt.plot(timeline, group['Combined Memory'], label=label,color=distinct_colors[idx % len(distinct_colors)], linestyle=line_styles[idx // 4 % len(line_styles)])
    handling.append((group['Pruning Ratio'].iloc[0],group['Combined Memory'].iloc[0]))
...
plt.xlabel('Timeline')
plt.ylabel('Combined Memory')
plt.title('Combined Memory Usage of different Pruning Ratio')
plt.legend()
plt.grid(True)
...
plt.savefig('combined_memory_usage_plot.png', bbox_inches='tight')
