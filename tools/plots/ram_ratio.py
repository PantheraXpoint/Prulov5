...
for ff in directory_path.glob('*'):
    ...
    if 'hardware' in file:
        ...
        mean_ram_usage = df.groupby('PID')['RAM Usage (MB)'].transform('mean')

        # Calculate the RAM usage difference with the mean
        df['RAM Usage Diff'] = df['RAM Usage (MB)'] - mean_ram_usage

        # Filter out rows where RAM usage difference is greater than 10 MB
        filtered_df = df[df['RAM Usage Diff'].abs() <= 10]

        # Drop the 'RAM Usage Diff' column as it's no longer needed
        filtered_df = filtered_df.drop(columns=['RAM Usage Diff'])
...
for idx,(pruning_ratio, group) in enumerate(grouped):
    ...
    line, = plt.plot(timeline, group['RAM Usage (MB)'], label=label,color=distinct_colors[idx % len(distinct_colors)], linestyle=line_styles[idx // 4 % len(line_styles)])
    handling.append((group['Pruning Ratio'].iloc[0],group['RAM Usage (MB)'].iloc[0]))
...
plt.title('RAM Usage of different Pruning Ratio')
plt.legend()
plt.grid(True)
...
plt.savefig('ram_usage_plot.png', bbox_inches='tight')
